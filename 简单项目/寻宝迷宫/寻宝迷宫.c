...
{
//(1)打开文件 把地图复制到二维数组中
	char str[100][100],judge;                 //str 二维数组保存地图  judge判断 
	int cnt=2,i;                            //记录关卡 
	system("color 3");                     //浅蓝色 
	...
	Sleep(2000);                        //延迟  清屏 
	system("cls"); 
	openmap(str,cnt);                      //打开文件和地图         
    updatemap(str,cnt);                     //更新地图1
...
{
	int i,j; 					        //i j 控制二维数组循环  地图行line 列 list 
	FILE *fp;							 //fp 文件指针 op 地图字符 file 保存地图文件路径 p指针选择 
	char op,file2[100]="D:\\代码程序\\2022大一c语言\\寻宝迷宫\\map2.txt",
	file1[100]="D:\\代码程序\\2022大一c语言\\寻宝迷宫\\map1.txt",*p;
	if(cnt==1)
	{
		p=file1;
		line=21;
	...
	fp=fopen(p,"r"); 						     //1.1打开文件 
	if(fp==NULL)								 //验证读取是否 
	{
		printf("打开失败");
		exit(0); 
	}
	for(i=0;i<line;i++)                          //1.2复制文件 
	{
		for(j=0;j<list;j++)
		{
			op=fgetc(fp);						 //读取字符 
